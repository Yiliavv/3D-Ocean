...
from research.plot import plot_sst_distribution, \
    plot_profile_for_predicted_in_lat, plot_compared_profile_for_predicted
...
# ------------------------------------ 预测海温数据 ------------------------
pre_stations = np.array([(i, j) for i in np.arange(160, 180) for j in np.arange(-19, 1)])
...
pres_sst = pres_result[:, 0].reshape(20, 20)

Log.d("预测数据生成完成: ", pres_result)
