...
class WrappedUNetLSTM(UNetLSTM):
    def __init__(self, width, height, seq_len, **kwargs):
        super().__init__(
            ...
            features=[32, 64, 128, 256], lstm_hidden_channels=128,
...
class WrappedTransformer(SSTTransformer):
    ...
    def forward(self, x):
        out = super().forward(x)  # [B, 1, width, height] = [B, 1, 180, 80]
        return out.squeeze(1).permute(0, 2, 1)  # [B, height, width] = [B, 80, 180]
...
def create_model(model_name: str, seq_len: int):
    ...
    if model_name == "RGTransformer":
        ...
        params.update({"hidden_dim": 64, "kernel_size": (3, 3), "num_layers": 2})
...
def preload_dataset(seq_len: int):
    """一次性将所有数据从磁盘加载到内存"""
    print(f"[Preload] Loading all data into memory (seq_len={seq_len})...")
...
@torch.no_grad()
def compute_r2(model, val_loader) -> float:
    """在验证集上计算 R² (coefficient of determination)"""
    device = next(model.parameters()).device
    model.eval()

    ss_res, ss_tot, y_sum, n = 0.0, 0.0, 0.0, 0

    for x, y in val_loader:
        x, y = x.to(device), y.to(device)
        y_pred = model(x)
        mask = ~(torch.isnan(y) | torch.isnan(y_pred))
        y_valid = y[mask].float()
        y_pred_valid = y_pred[mask].float()
        ss_res += ((y_valid - y_pred_valid) ** 2).sum().item()
        y_sum += y_valid.sum().item()
        n += y_valid.numel()
    ...
    for x, y in val_loader:
        ...
        mask = ~(torch.isnan(y) | torch.isnan(y_pred))
        y_valid = y[mask].float()
        ss_tot += ((y_valid - y_mean) ** 2).sum().item()
...
def run_experiment(exp_config: dict, exp_id: int, total: int) -> dict:
    ...
    print(f"\n[Result] Time: {train_time:.1f}s | TrainLoss: {best_train_loss:.6f} | ValLoss: {best_val_loss:.6f} | ValRMSE: {best_val_rmse:.4f} | R²: {val_r2:.4f}")
...
def save_results(results: list, output_path: str):
    ...
    fields = ["model", "epochs", "seq_len", "params", "train_time_sec", "best_train_loss", "best_val_loss", "best_val_rmse", "val_r2"]
...
def print_summary(results: list):
    ...
    print("=" * 130)
    print(f"{'Model':<16} {'Epochs':>7} {'SeqLen':>7} {'Params':>12} {'Time(s)':>10} {'TrainLoss':>12} {'ValLoss':>12} {'ValRMSE':>10} {'R²':>10}")
    print("-" * 130)
    for r in results:
        print(f"{r['model']:<16} {r['epochs']:>7} {r['seq_len']:>7} {r['params']:>12,} {r['train_time_sec']:>10.1f} {r['best_train_loss']:>12.6f} {r['best_val_loss']:>12.6f} {r['best_val_rmse']:>10.4f} {r['val_r2']:>10.4f}")
    print("=" * 130)
...
def main():
    ...
    print("=" * 70)
    print("SST Model Benchmark (5 models x 2 epochs x 2 seq_len = 20 runs)")
    print("=" * 70)
    ...
    output_path = f"{PROJECT_PATH}/out/benchmark_results.csv"
